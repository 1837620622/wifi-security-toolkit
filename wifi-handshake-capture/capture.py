...
def capture_netsh(ssid: str, bssid: str) -> CaptureResult:
    ...
    # 启动跟踪
    print("    [2/4] 启动网络跟踪（需管理员权限）...")
    try:
        result = subprocess.run(
            ['netsh', 'trace', 'start', 'capture=yes',
             f'traceFile={trace_file}', 'maxSize=10', 'overwrite=yes'],
...
def parse_etl(etl_file: str, ssid: str, bssid: str) -> CaptureResult:
    ...
    try:
                    ...
                    if eapol_end <= len(data):
                        ...
                        if len(fd) > 49:
                            ...
                            if has_ack and not has_mic:
                                m1_frames.append({'anonce': fd[17:49], 'raw': fd})
        ...
        return CaptureResult(error=f"ETL解析: M1={len(m1_frames)} M2={len(m2_frames)}")
